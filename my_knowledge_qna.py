...
def split_chunks(sources):
    chunks = []
    splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=32)
...
def similarity_search(query, index):
    ...
    # default is 4
    matched_docs = index.similarity_search(query, top_k=1) 
    sources = []
...
doc_list = [s for s in os.listdir(pdf_folder_path) if s.endswith('.txt')]
...
loader = TextLoader(os.path.join(pdf_folder_path, doc_list[0]))
...
for i in range(1,num_of_docs):
    ...
    loader = TextLoader(os.path.join(pdf_folder_path, doc_list[i]))
